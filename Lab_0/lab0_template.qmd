...
```{r setuup}
car_data <- read_csv("D:/Ph.D/US Pakistan Knowledge Corridore/Upenn/Subjects/Public Policy Analytics/Public-Policy-Analytics/Lab_0/data/car_sales_data.csv")
```
...
selected_datadd <- car_df %>%
  select(-"Engine size")
View(selected_datadd)
```
...
ss <- rename(car_data, YY = "Engine size" )
View(ss)
...
filtered_data <- filter(GG, Manufacturer == "Honda"  | Manufacturer == "Nisan" )
